...
public class GenerateChessBoardWorldCorners_offsetStart {
	...
	public GenerateChessBoardWorldCorners_offsetStart(int rows,int cols,float squareSize,int numImages,List<Mat> objectPoints)
	{
		...
		for (int i = 0; i < rows; i++) 
		{
			for (int j = 0; j < cols; j++)
			{
				...
				index = (i*cols + j);
				location[0] = ((j+1)*squareSize);
				location[1] = ((i+1)*squareSize);
				location[2] = 0;
